...
while ix < 7 and strMiSueldo >= sueldos[ix]:
    ix += 1
...
if ix == 0:
    ...
    retencion = retenciones[ix-1] + (((strMiSueldo - sueldos[ix-1]) * (retenciones[ix] - retenciones[ix-1]))/(sueldos[ix] - sueldos[ix-1]))

retencionTotal = retencion + 6.35

miSueldoNeto = strMiSueldo * (100 - retencionTotal)/100
